...
custom_test_1() {
    ...
    tmp_score=100
    for i in {0..5}; do
        filename=".admin_files/tmpfileforct1$RANDOM.sec"
        rm "$filename"&> /dev/null
        
        timeout "$process_timeout" ./target/debug/gen_keys "$filename"

        size_results=($(wc -c "$filename")) 
        rm "$filename"

        if [[ "${size_results[0]}" != "12800" ]]; then
            tmp_score=0
        fi
    done;
