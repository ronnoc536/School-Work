...
def test() -> bool:
    ...
    other_adminsmiths = [
        characters.AdminSmith(party_row + (100 * i), party_col + (100 * i))
        for i in range(random.randint(3, 7))
    ]
    result = True
    for case in cases:
        for other_adminsmith in other_adminsmiths:
            adminsmith.row = other_adminsmith.row + (case[0] * random.randint(10, 50))
            adminsmith.col = other_adminsmith.col + (case[1] * random.randint(10, 50))
