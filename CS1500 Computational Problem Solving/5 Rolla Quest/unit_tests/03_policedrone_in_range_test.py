...
def test() -> bool:
    ...
    # Needs the previous test case to confirm policedrone does not do nothing
    oracle_row, oracle_col = 200, 200
    oracle = characters.The0racle(oracle_row, oracle_col)
    policedrone = characters.PoliceDrone(oracle_row, oracle_col, oracle)
    result = True
    moves = {
        KEY_DICT["up"]: (-1, 0),
        KEY_DICT["left"]: (0, -1),
    ...
    for _ in range(random.randint(50, 100)):
        direction = moves[policedrone.move()]
        policedrone.row += direction[0]
        policedrone.col += direction[1]
        if oracle.distance(policedrone) > 5:
            result = False
    return result
